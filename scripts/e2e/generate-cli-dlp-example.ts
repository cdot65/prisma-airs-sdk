...
const installed = read('artifacts/cli-431-registry-verification.json');
assert.equal(installed.passed, true);
assert.equal(installed.version, '4.3.1');
assert.equal(native.cliVersion, installed.version);
assert.equal(native.sdkVersion, '0.24.0');
...
for (const path of [
  'artifacts/security/cli431-production-audit.json',
  'artifacts/security/cli431-registry-production-audit.json',
]) {
