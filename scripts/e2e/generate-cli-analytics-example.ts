...
const name = `cli-analytics${selection.suffix}`;
const capture = JSON.parse(readFileSync(resolve(root, `artifacts/examples/${name}.json`), 'utf8'));
const report = JSON.parse(readFileSync(resolve(root, `artifacts/e2e/${name}.json`), 'utf8'));
...
assert.equal(capture.output.version, selection.cliVersion);
assert.equal(capture.output.sdkVersion, selection.sdkVersion);
