...
<img src={useBaseUrl('/img/logo-banner.svg')} alt="Prisma AIRS SDK" width="460" />
...
Type-safe clients for all four AIRS service domains — real-time content scanning, security configuration management, model security analysis, and AI red teaming. Zero external HTTP dependencies. Native `fetch` + `crypto` for transport, with `zod` for response validation. ESM-first with dual CJS/ESM exports.
...
- **Model Security** — Scan ML models for supply chain threats: malicious code execution, backdoors, unapproved file formats. Browse model/version inventory, manage security groups and rule instances, and inspect read-only platform rules.
- **AI Red Teaming** — Run automated red team scans against AI targets. Static attack libraries, dynamic agent-based testing, custom prompt sets, network broker channels, and comprehensive reporting.
...
- **[Develop](developer/development)** — Local commands, docs workflow, CI, preflight, and release notes.
