...
The SDK is modeled directly from Palo Alto Networks' OpenAPI specifications. In this repository the
local specs are expected under a gitignored `schemas/` alias, usually as symlinks into a local
pan.dev `openapi-specs` checkout:
...
hand-written, runtime-validating counterpart, and the local preflight check (below) compares the
two when the `schemas/` alias exists.
...
The external DLP spec tree contains more yaml than the SDK models. The preflight script lists only
the four DLP specs that are actually implemented (`DataFilteringProfiles`, `DataPatterns`,
`DataProfiles`, `Dictionaries`). Loading the rest would create component-name collisions (for
example, multiple `Policy` definitions) and produce false drift against unrelated schemas.
:::
...
1. Loads and dereferences the explicit `MODELED_SPECS` list from the gitignored `schemas/` alias
   into a map of component schemas.
...
latter fails a strict run.

Because `schemas/` is gitignored, preflight is a **local / pre-release check**, not a GitHub Actions
CI gate. Run it before tagging a release or after API-side changes, and keep the `MODELED_SPECS`
array in `scripts/preflight-schemas.ts` in sync with the Zod models.
