...
preflight's strict checks on required fields and shared field types are what guard against that.
...
The AI Gateway has no published spec; its models are verified against live SCM responses instead
(see the [AI Gateway guide](../guides/ai-gateway-api.mdx)).
...
`npm run preflight` (`scripts/preflight-schemas.ts`) is the static drift check. It:

1. Loads and dereferences each spec in `MODELED_SPECS` from `schemas/` into a map of component
   schemas (and exits 2 if the alias or any listed file is missing).
...
```bash
npm run preflight        # strict — exit 1 on unacknowledged drift (local / pre-release)
...
latter fails the run. The check started life as `preflight:warn` in CI while drift was paid down
(81 → 39 findings across a series of PRs), flipped to strict, and was later moved out of CI because
the specs are a gitignored local checkout. Today it is a **local, pre-release check**: run
`npm run preflight` before tagging a release or after API-side changes.
