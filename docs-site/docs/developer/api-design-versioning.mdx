...
`package.json` is **`0.13.2`** — i.e. **pre-1.0**.
...
The SDK is modeled directly from Palo Alto Networks' OpenAPI specifications. Those specs are not
checked into this repository. For local release checks, `scripts/preflight-schemas.ts` reads a
gitignored `schemas/` alias made of symlinks to a local pan.dev `openapi-specs` checkout:
...
The exact list is the `MODELED_SPECS` array in `scripts/preflight-schemas.ts`. Keep that list in
sync with the Zod models under `src/models/`. The OpenAPI specs are the **source of truth for
shapes**; the Zod schemas are the hand-written, runtime-validating counterpart.
...
:::note[Why the preflight list is explicit]
The local `schemas/` alias can contain `-latest` duplicates and unmodeled APIs, including more DLP
specs than the SDK currently implements. Loading the whole tree would create component-name
collisions (for example, unrelated `Policy` definitions) and false drift. The script therefore loads
only the services represented in `src/models/`.
:::
## Preflight: catching Zod-vs-OpenAPI drift before release

`npm run preflight` (`scripts/preflight-schemas.ts`) is the static drift gate. It:

1. Loads and dereferences the `MODELED_SPECS` files under `schemas/` into a map of component
   schemas.
...
latter fails the command. Because `schemas/` is gitignored and depends on a local external checkout,
preflight is a local / pre-release check rather than a CI gate. Run it before tagging a release or
