...
`package.json` is **`0.17.0`** — i.e. **pre-1.0**.
...
Most SDK schemas are modeled directly from Palo Alto Networks' OpenAPI specifications. For local
pre-release checks, this repository expects a gitignored `schemas/` directory of symlinks to a local
`openapi-specs` checkout:
...
These specs are the **source of truth for shapes** where a published spec exists. The Zod schemas in
`src/models/` are the hand-written, runtime-validating counterpart, and the preflight script (below)
checks that the modeled specs and implemented schemas agree.
...
**extends beyond** it: the Python SDK covers scanning, whereas this SDK also covers Management and
DLP, Model Security, AI Red Teaming, and AI Gateway.

:::note[AI Gateway is unspecced]
The AI Gateway API does not currently have a published OpenAPI spec in the local preflight set.
Its schemas live in `src/models/ai-gateway.ts` and are excluded from `npm run preflight` by the
`UNSPECCED_MODEL_FILES` list in `scripts/preflight-schemas.ts`. Use `npm run smoke:ai-gateway`
against a live tenant when you need an end-to-end schema check for that subsystem.
:::

:::note[Why some DLP specs are excluded]
`schemas/dlp/` can contain more yaml than the SDK models. The preflight script whitelists only the
...
:::
## Preflight: catching Zod-vs-OpenAPI drift locally

`npm run preflight` (`scripts/preflight-schemas.ts`) is the static drift check. It:

1. Loads and dereferences the explicitly modeled OpenAPI specs linked under `schemas/` into a map of
   component schemas.
...
```bash
npm run preflight        # strict — exit 1 on unacknowledged drift
...
latter fails the command. Because `schemas/` is gitignored local state, preflight is a local /
pre-release check rather than a GitHub Actions gate. Run `preflight` before tagging a release or
after API-side schema changes.
