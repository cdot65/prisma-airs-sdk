...
| `mcpServers.updateCapabilities` | Owned MCP discovery remains blocked before tool listing: the faithful source-configuration clone returned HTTP 401 for a required caller authentication header (2026-09-07). Earlier minimal clones returned gateway HTTP 500 after an upstream unauthorized response. No tool was invoked; updateCapabilities remains contract-tested only. |
