...
- **401/403 handling**: OAuth2 clients refresh the token and retry at most once, outside the retry budget. An explicit SCM policy denial (`403` with `x-opa-decision: false`) is not retried: refreshing the same identity cannot repair its permissions.
