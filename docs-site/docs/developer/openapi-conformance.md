...
The SDK **0.25.0 candidate** additionally implements the shared filters on all six grouped analytics endpoints. Full verification passes **11,263 tests in 142 files**, with **99.71% lines/statements, 100% functions and 96.67% branches**. Raw SCM and independently installed-candidate group checks each pass **102/102**, including positive/absent cohorts, CSV-OR lists, inclusive ranges, intersections and preserved cost/token columns. Candidate inference passes **10/10** at **2026-09-07T11:29:54.106Z**; independent retirement passes **32/32** at **2026-09-07T11:30:54.151Z**. See [captured group output](../guides/examples.mdx#verified-grouped-analytics-filters) and [typed group options](../guides/ai-gateway-api.mdx#groupby-byuser-bystatuscode). Source fixtures record provider `traceId` as an SCM-only extension because the pinned provider operation omits `trace_id`. Single-read regressions also prove chart/group/log serialization uses the validated copy rather than rereading caller-owned getters. Package and strict consumer-type verification pass, using a tracked specimen; registry publication and CLI group-filter integration are separate pending gates. Coverage remains **138/242 direct**, and the full-project assessment remains **5/10**. The published 0.24.0/4.3.1 checkpoint below is preserved separately.
