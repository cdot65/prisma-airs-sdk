...
The latest verification-only checkout passes **11,313 tests in 145 files**, with **99.71% lines/statements, 100% functions and 96.67% branches**. Its 50 additional harness regressions cover versioned package preflight/history, secret-safe subprocess failures and bounded administration route plans; the published 0.25.0 runtime is unchanged. The live inventory preserves **72 timestamped suites**, including separate registry CLI 4.4.0 runs and all previously recorded failures; these are workflow-check counts, not unique covered operations.

At **2026-09-07T13:09:51.744Z**, both SCM workspace authentication controls pass, but all **24** corrected authoring/administration GET hypotheses return OPA-denied **HTTP 403**. The probe includes required virtual-key pagination, camel-case user/invitation pagination and both applicable workspace-member path prefixes. It changes no identities, memberships, permissions or keys, and retains no response bodies. See [every actual status/header projection](../guides/examples.mdx#administration-route-availability). This narrows the next prerequisite to verified Prisma routes and authorization contracts; it does not establish feature absence, add covered operations or satisfy the 99% target.
