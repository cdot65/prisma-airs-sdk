...
SDK **0.24.0** and CLI **4.3.0** are published, independently installed from npm and payload-verified. The SDK release passes **10,931 tests in 136 files**; three subsequent installation-preflight regressions bring the verification checkout to **10,934 tests in 137 files**, with 99.71% lines/statements, 100% functions and 96.55% branches. CLI validation passes **1,127 tests in 90 files**. Seven additional filters preserve SCM casing, CSV-OR lists, inclusive bounds, zero and fractional cents; the CLI uses the exported SDK schema before client creation or cost workspace resolution. Cost UUID resolution and malformed day parsing are corrected without changing its cents/USD output.

Registry SDK query contracts pass **53/53** at **2026-09-07T08:56:03.088Z**; registry CLI query contracts pass **54/54** at **2026-09-07T09:20:57.909Z**. SDK inference passes **10/10**, CLI inference **8/8**, empty-window CLI output **3/3**, and upgraded user-install cross-service reads/scan **12/12**. Independent retirement verifies **28/28** historical release-inference keys absent. ESM/CommonJS payload, strict-type, OAuth and native WebSocket verification covers all 1,304 SDK runtime exports. See [actual SDK query output](../guides/examples.mdx#verified-chart-query-contracts), [CLI query output](https://cdot65.github.io/prisma-airs-cli/cli/aigateway/telemetry/#verified-chart-filter-output), and [version-specific registry results](../guides/release-verification.md#sdk-0240-and-cli-430-registry-verification). Direct coverage stays **138/242**, with all 22 analytics operations partial and the full-scope assessment **5/10**.

The SDK production audit is clean; the separate CLI production lockfile audit is not: five high and one moderate advisory affect YAML, ID generation and optional image dependencies. Fresh npm resolution leaves optional sharp's libvips advisory and its package-level propagation. These require a focused dependency/native-image verification follow-up, not suppressed findings. Existing service/provider failures, the retained DLP fixture and WAN/container-runtime limitations remain unchanged.
