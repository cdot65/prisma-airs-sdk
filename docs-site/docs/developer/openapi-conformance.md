...
The next SDK 0.24.0 query-contract increment passes **10,931 local tests in 136 files**, with 99.71% lines/statements, 100% functions and 96.55% branches. Seven additional filters preserve SCM casing, CSV-OR lists, inclusive bounds, zero and fractional cents; an exported shared schema supports CLI validation before workspace resolution/authentication. Raw SCM validation passes **53/53** at **2026-09-07T08:12:49.467Z**; the final packed candidate independently passes **53/53** at **2026-09-07T08:36:44.862Z**. ESM/CommonJS payload, strict-type, OAuth and native WebSocket verification passes at **2026-09-07T08:37:24.322Z**, covering all 1,304 runtime exports. These are candidate results, not a registry-publication claim. See [actual query-contract output](../guides/examples.mdx#verified-chart-query-contracts). Direct coverage stays **138/242**, with all 22 analytics operations still partial and the full-scope assessment still **5/10**.
