...
The checked-in `.env.example` includes the core scan and management credentials. Some optional
endpoint overrides listed below are documented from source constants and may need to be added to
your local `.env` manually.
...
[airs-sdk]   headers {"User-Agent":"PAN-AIRS/0.13.2-typescript-sdk","service-name":"api","Authorization":"sha256:6ba877dddebe"}
