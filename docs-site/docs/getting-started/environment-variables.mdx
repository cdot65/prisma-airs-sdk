...
> AI Gateway operations redact fields listed in `AI_GATEWAY_SECRET_FIELDS`; unknown fields and
> other service bodies are logged verbatim and may contain prompt content. Keep debug logging off
> in production and review captured output before sharing.
