...
| **API key** or **bearer token** | AI Runtime Security content scans only                                   |
...
graph LR
    ...
    subgraph "API key or bearer token"
        SCAN[AI Runtime Security<br/>Content Scanning]
    end
```

In practice, if you set `PANW_MGMT_CLIENT_ID`, `PANW_MGMT_CLIENT_SECRET`, and `PANW_MGMT_TSG_ID`, all OAuth2 services share those credentials automatically. AI Runtime Security content scanning is configured separately through `init()` with `PANW_AI_SEC_API_KEY`, `PANW_AI_SEC_API_TOKEN`, or explicit options.
...
This is the **only service configured through `init()`** instead of OAuth2. It accepts an API key, a pre-obtained bearer token, or both.

```ts
import { init, Scanner, Content } from '@cdot65/prisma-airs-sdk';

// Initialize with an API key or pre-obtained bearer token (not OAuth2)
...
# AI Runtime Security (API key or bearer token auth)
