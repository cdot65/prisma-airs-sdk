...
All SDK clients can be configured via constructor options or environment variables. Copy
`.env.example` to `.env` to get started with the core credentials; add optional endpoint overrides
from the tables below when your tenant needs them.
