...
| `PANW_MGMT_ENDPOINT`       | No       | `https://api.apps.paloaltonetworks.com/aisec`                |
...
| `PANW_MODEL_SEC_DATA_ENDPOINT`  | No                 | `https://api.apps.paloaltonetworks.com/aims/data` |
| `PANW_MODEL_SEC_MGMT_ENDPOINT`  | No                 | `https://api.apps.paloaltonetworks.com/aims/mgmt` |
...
| `PANW_RED_TEAM_DATA_ENDPOINT`  | No                 | `https://api.apps.paloaltonetworks.com/ai-red-teaming/data-plane` |
| `PANW_RED_TEAM_MGMT_ENDPOINT`  | No                 | `https://api.apps.paloaltonetworks.com/ai-red-teaming/mgmt-plane` |
| `PANW_RED_TEAM_NETWORK_BROKER_ENDPOINT` | No        | `https://api.apps.paloaltonetworks.com/ai-red-teaming/data-plane/network-broker` |
...
Set `PANW_AI_SEC_DEBUG` to `1`, `true`, `yes`, or `on` to log each HTTP request and response to `stderr`. The SDK hashes `Authorization` and `x-pan-token` values. For AI Gateway operations with published secret metadata it also redacts known provider credentials, plugin credentials, auth settings, and one-time response secrets. Other body content is logged verbatim, so keep this disabled in production.
