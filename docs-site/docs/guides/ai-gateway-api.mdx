...
The provider detail response can contain `key`, `model_config` service-account material, and `secret_mappings`. Do not log or persist the complete response, and do not enable `PANW_AI_SEC_DEBUG` around this call in production.
:::
...
for a targeted update; setting it to `true` applies replacement semantics to the supplied binding
set.
