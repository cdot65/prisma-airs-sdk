...
The AI Gateway client uses OAuth2 `client_credentials`, exactly like every other client in this SDK. The `PANW_AI_GW_CLIENT_ID` / `_CLIENT_SECRET` / `_TSG_ID` / `_TOKEN_ENDPOINT` variables fall back to the corresponding `PANW_MGMT_*` ones if unset (the two endpoint overrides have no fallback), so an account already configured for `ManagementClient` needs nothing extra beyond the two SCM grants above.
...
The request-count chart additionally accepts semantically verified filters:
...
`AIGatewayRequestChartOptions` exports this contract. SCM respects camel-case `traceId`, but ignores upstream `trace_id`; metadata is encoded as JSON with exact string-valued matches. Known and nonexistent synthetic filters were tested separately, not inferred from HTTP 200. See [actual captured filter output](./examples.mdx#verified-request-chart-filters). Other chart methods do not accept these filters until their semantics are independently verified.

Telemetry options are validated before authentication/network I/O. Unknown fields, invalid dates, non-finite days, reversed windows, malformed log options and unsupported grouping dimensions/columns raise `USER_REQUEST_PAYLOAD_ERROR` without including rejected values. Fractional days remain valid; explicit `start` overrides finite `days`. Dates retain numeric UTC offsets required by SCM. A zero-length explicit window is allowed; no unverified server maximum is imposed.
