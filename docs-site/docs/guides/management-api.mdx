...
**Tune profiles from real traffic.** Use `scanLogs.query({ time_interval, time_unit, pageNumber: 1, pageSize: 50, filter: 'threat' })` to see what a profile actually caught (or missed), then adjust detectors and custom topics. Close the loop instead of guessing.
