...
description: How to run the SDK example scripts for scanning, management, DLP, model security, red teaming, AI Gateway, and validation workflows.
---

# Runnable Examples

The repository includes runnable TypeScript examples in `docs-site/examples/` and a read-only AI
Gateway walkthrough in `examples/ai-gateway.ts`. They exercise the same public SDK APIs documented
in the guides and can be run with `tsx` from the repository root.
...
- The AI Gateway example can use `PANW_AI_GW_*` credentials or fall back to `PANW_MGMT_*`, and the
  service account needs both AI Gateway SCM role grants described in the
  [AI Gateway guide](./ai-gateway-api).
...
| Script | What it demonstrates |
| --- | --- |
| `examples/ai-gateway.ts` | Read-only telemetry, workspace, config-plane, admin-plane, audit-log, and error-handling calls through `AIGatewayClient`. |

The AI Gateway example lives under the repository-level `examples/` directory because it covers the
full live-tenant workflow and is excluded from the published package.
